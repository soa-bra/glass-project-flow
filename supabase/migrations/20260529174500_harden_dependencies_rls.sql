...
--
-- Supabase guidance: keep SECURITY DEFINER helpers outside exposed schemas.

create schema if not exists private;

create or replace function private.can_access_central_entity(
...
revoke all on function private.can_access_central_entity(public.central_entity_type, uuid) from public;
grant execute on function private.can_access_central_entity(public.central_entity_type, uuid) to authenticated;
...
create policy "users can view accessible dependencies"
  ...
  to authenticated
  using (
    private.can_access_central_entity(from_entity_type, from_entity_id)
    and private.can_access_central_entity(to_entity_type, to_entity_id)
  );
