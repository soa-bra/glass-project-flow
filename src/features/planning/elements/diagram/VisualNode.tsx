...
const VisualNode: React.FC<VisualNodeProps> = ({
  ...
  const nodeData = element.data as VisualNodeData;
  ...
  const handleAnchorMouseDown = useCallback((
    ...
    e.stopPropagation();
    const pos = getVisualAnchorPosition(element.position, element.size, anchor);
  ...
  return (
    <div
      ...
      } ${isSelected ? 'ring-2 ring-[hsl(var(--accent-blue))] ring-offset-2' : ''}`}
      style={{
      ...
      >
        ...
        {nodeData.isRoot && (
          <div className="absolute -top-2 -right-2 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow-sm">
            <div className="w-3 h-3 rounded-full bg-[hsl(var(--accent-blue))]" />
          </div>
        )}
        ...
        {hasChildren && (
          <button
            ...
            title={nodeData.isCollapsed ? "توسيع" : "طي"}
          >
      ...
      {(isSelected || isConnecting) && (
        <>
          {(['top', 'bottom', 'left', 'right'] as const).map((anchor) => {
            const pos = getVisualAnchorPosition({ x: 0, y: 0 }, element.size, anchor);
              ...
              <div
                ...
                className={`absolute w-4 h-4 rounded-full border-2 transition-all cursor-crosshair ${
                  isHighlighted
                    ...
                    : 'bg-white border-[hsl(var(--ink-30))] hover:border-[hsl(var(--accent-blue))] hover:scale-110'
                }`}
                style={{
                  left: pos.x - 8,
                  top: pos.y - 8,
