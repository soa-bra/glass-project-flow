...
} from '../../adapters/departmentBoxesAdapter';
...
const SmartElementsPanel: React.FC = () => {
  ...
  const departmentBoxSources = getDepartmentBoxSources();
  const filteredBoxSources = departmentBoxSources.filter((box) => {
  ...
  const boxSourcesByDepartment = filteredBoxSources.reduce<Record<string, DepartmentBoxSource[]>>((acc, box) => {
    acc[box.departmentLabel] = [...(acc[box.departmentLabel] ?? []), box];
    return acc;
  }, {});
  const shouldShowBoxes = normalizedSelectedCategory === 'all' || normalizedSelectedCategory === 'boxes';

  const filteredElements = normalizedSelectedCategory === 'all' 
    ? SMART_ELEMENTS 
    : SMART_ELEMENTS.filter(el => normalizeCategoryId(el.category) === normalizedSelectedCategory);
