...
const UnifiedFloatingToolbar: React.FC = () => {
  ...
  const isMindmapSelection = useMemo(() => {
    return selectedElements.some((el) => el.type === "mindmap_node" || el.type === "mindmap_connector");
  }, [selectedElements]);
  ...
  const handleAddText = () => {
    ...
    const centerX =
      activeElements.length > 0
        ? activeElements.reduce((sum, el) => sum + (el.position.x + (el.size?.width || 200) / 2), 0) /
  ...
  const CommonActions = () => (
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          className="w-48 bg-white z-[10001] pointer-events-auto"
          onMouseDown={(e) => e.stopPropagation()}
        >
  ...
  const TextActions = () => (
    <>
      ...
      {/* لون النص */}
      <ColorButton value={currentColor} onChange={handleColorChange} icon={<Type size={14} />} title="لون النص" />
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent
          className="bg-white z-[10001] min-w-0 p-1 pointer-events-auto"
          onMouseDown={(e) => e.stopPropagation()}
        >
          <div className="flex gap-1">
            <button
              ...
              className={cn(
                ...
                currentAlign === "right" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
              <AlignRight size={16} />
            </button>
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentAlign === "center" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
              <AlignCenter size={16} />
            </button>
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentAlign === "left" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
              <AlignLeft size={16} />
            </button>
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentAlign === "justify" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
        ...
        >
          <div className="flex gap-1">
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentVerticalAlign === "flex-start" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentVerticalAlign === "center" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
            ...
            <button
              ...
              className={cn(
                "w-8 h-8 flex items-center justify-center rounded hover:bg-[hsl(var(--ink)/0.1)]",
                currentVerticalAlign === "flex-end" && "bg-[hsl(var(--ink)/0.1)]",
              )}
            >
  ...
  const ElementActions = () => {
    const currentOpacity =
      firstElement?.style?.opacity !== undefined
    ...
    const currentStrokeWidth =
      firstElement?.style?.borderWidth !== undefined
      ...
      selectedElementIds.forEach((id) => {
        updateElement(id, {
          style: {
            ...firstElement?.style,
            borderColor: color,
            borderWidth: currentStrokeWidth || 1,
            borderStyle: "solid",
          },
        });
      });
  ...
  const MindmapActions = () => {
    ...
    // تطبيق التخطيط
    const handleLayoutSettingChange = useCallback(
    ...
    const handleDuplicateMindmap = useCallback(() => {
      ...
      // أولاً: تكرار العقد
      mindmapTreeElements
        .filter((el) => el.type === "mindmap_node")
        .forEach((node) => {
    ...
    return (
      <>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-[hsl(var(--accent-blue)/0.1)] to-[hsl(var(--accent-green)/0.1)] rounded-lg">
          ...
          <span className="text-xs font-semibold text-[hsl(var(--ink))] max-w-[120px] truncate">{mindmapName}</span>
        </div>
