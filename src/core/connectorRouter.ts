...
import { canvasGraph, type GraphNode, type GraphEdge, type EdgeEndpoint } from './canvasGraph';
import type { Point } from './canvasKernel';
...
class ConnectorRouterImpl {
  ...
  route(
    ...
    const startPoint = this.resolveEndpoint(source);
    const endPoint = this.resolveEndpoint(target);
  ...
  /**
   * تحديد موقع نقطة النهاية
   */
  private resolveEndpoint(endpoint: EdgeEndpoint): Point {
    if (endpoint.nodeId && endpoint.anchor) {
      const anchorPos = canvasGraph.getAnchorPosition(endpoint.nodeId, endpoint.anchor);
