...
export const TransactionsTab: React.FC = () => {
  const getTransactionStatusColor = (status: string) => {
    switch (status) {
      case 'completed': return 'bg-[#bdeed3] text-black';
      case 'pending': return 'bg-[#fbe2aa] text-black';
      case 'cancelled': return 'bg-[#f1b5b9] text-black';
      case 'processing': return 'bg-[#a4e2f6] text-black';
        ...
        <div className="px-0">
          <div className="space-y-3">
            {mockTransactions.map(transaction => (
              <div key={transaction.id} className="flex items-center justify-between p-4 bg-transparent border border-black/10 rounded-3xl">
