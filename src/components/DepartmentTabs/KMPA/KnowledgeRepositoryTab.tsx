...
export const KnowledgeRepositoryTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <BaseBox>
        <div className="mb-4">
          <h3 className="text-lg font-semibold flex items-center gap-2"><Search className="h-5 w-5" /> البحث والتصفية</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div><Input placeholder="البحث في الوثائق..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full" /></div>
      ...
      <BaseBox>
        <div className="flex items-center justify-between">
          ...
          <BaseActionButton className="flex items-center gap-2"><Upload className="h-4 w-4" /> رفع وثيقة</BaseActionButton>
        </div>
      </BaseBox>
      ...
      <AppDashboardGrid columns={12} minRowHeight="auto">
        {filteredDocuments.map((doc) => (
          <AppGridItem key={doc.id} colSpan={4} tabletSpan={3}>
            <BaseBox className="hover:shadow-lg transition-shadow h-full">
              <div className="mb-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <BaseBadge variant={getTypeBadgeVariant(doc.type)} className="text-xs">
                        {doc.type === 'research' ? 'بحث' : doc.type === 'publication' ? 'منشور' : doc.type === 'report' ? 'تقرير' : doc.type === 'guide' ? 'دليل' : doc.type === 'template' ? 'نموذج' : 'مقياس'}
                      </BaseBadge>
                      <BaseBadge variant={getStatusBadgeVariant(doc.status)} className="text-xs">
                        {doc.status === 'published' ? 'منشور' : doc.status === 'draft' ? 'مسودة' : doc.status === 'review' ? 'مراجعة' : 'مؤرشف'}
                      </BaseBadge>
                    </div>
                  </div>
                  <BaseActionButton variant="ghost" size="sm"><Star className="h-4 w-4" /></BaseActionButton>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <User className="h-3 w-3" /><span>{doc.author}</span>
                  <Calendar className="h-3 w-3 mr-1" /><span>{new Date(doc.createdAt).toLocaleDateString('ar-SA')}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Eye className="h-3 w-3" /><span>{doc.readCount}</span>
                  <Download className="h-3 w-3 mr-1" /><span>{doc.downloads}</span>
                </div>
                <div className="flex flex-wrap gap-1">
                  {doc.tags.slice(0, 3).map((tag, index) => <BaseBadge key={index} variant="outline" className="text-xs">{tag}</BaseBadge>)}
                  {doc.tags.length > 3 && <BaseBadge variant="outline" className="text-xs">+{doc.tags.length - 3}</BaseBadge>}
                </div>
                <div className="flex gap-2 pt-2">
                  <BaseActionButton size="sm" variant="outline" className="flex-1"><Eye className="h-3 w-3 mr-1" /> عرض</BaseActionButton>
                  <BaseActionButton size="sm" variant="outline" className="flex-1"><Download className="h-3 w-3 mr-1" /> تحميل</BaseActionButton>
                </div>
              </div>
            </BaseBox>
          </AppGridItem>
