...
export const OverviewTab: React.FC = () => {
  ...
  return <div className={`space-y-6 ${SPACING.SECTION_MARGIN}`}>
      ...
      gap: '1.5rem'
        ...
        {/* إحصائيات العقود */}
        <UnifiedCard title="توزيع العقود حسب الحالة" icon={<FileText className={LAYOUT.ICON_SIZE} />}>
          <UnifiedStatsCard stats={[{
          title: 'موقعة',
          value: metrics.contractsCount.signed
        }, {
          title: 'في الانتظار',
          value: metrics.contractsCount.pending
        }, {
          title: 'منتهية',
          value: metrics.contractsCount.expired
        }, {
          title: 'قيد المراجعة',
          value: metrics.contractsCount.underReview
        }]} columns={2} />
        </UnifiedCard>

        {/* درجة الامتثال ومؤشرات المخاطر */}
        <UnifiedCard title="مؤشرات الامتثال والمخاطر" icon={<Shield className={LAYOUT.ICON_SIZE} />}>
      ...
      gap: '1.5rem'
        ...
        {/* الإحصائيات الشهرية */}
        <UnifiedCard title="الإنجازات الشهرية" icon={<TrendingUp className={LAYOUT.ICON_SIZE} />}>
          <UnifiedStatsCard stats={[{
          title: 'عقد موقع',
          value: metrics.monthlyStats.contractsSigned
        }, {
          title: 'قضية محلولة',
          value: metrics.monthlyStats.casesResolved
        }, {
          title: 'فحص امتثال',
          value: metrics.monthlyStats.complianceChecks
        }, {
          title: 'تقييم مخاطر',
          value: metrics.monthlyStats.riskAssessments
        }]} columns={2} />
        </UnifiedCard>

        {/* التنبيهات القانونية */}
        <UnifiedCard title="تنبيهات قانونية عاجلة" icon={<Bell className={LAYOUT.ICON_SIZE} />}>
          <div className="space-y-3">
            {alerts.map(alert => <UnifiedListItem key={alert.id} icon={alert.priority === 'high' ? <AlertTriangle className={LAYOUT.ICON_SIZE} /> : alert.priority === 'medium' ? <Clock className={LAYOUT.ICON_SIZE} /> : <CheckCircle className={LAYOUT.ICON_SIZE} />} badge={{
