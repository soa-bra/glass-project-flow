...
export const ReportsTab: React.FC = () => {
  ...
  const getTypeConfig = (type: string) => reportTypes.find(t => t.value === type) || { icon: FileText, color: 'gray', label: type };
  const getStatusColor = (status: string) => {
    switch (status) { case 'جاهز': return 'bg-green-100 text-green-800'; case 'قيد الإنشاء': return 'bg-yellow-100 text-yellow-800'; case 'خطأ': return 'bg-red-100 text-red-800'; default: return 'bg-gray-100 text-gray-800'; }
  };
  ...
  const handleCreateReport = () => {
    ...
    const report = { id: `r-${Date.now()}`, title: newReport.title, type: newReport.type, description: 'تقرير مخصص', lastGenerated: new Date().toISOString().split('T')[0], frequency: 'مرة واحدة', status: 'جاهز', format: [newReport.format.toUpperCase()], insights: ['تقرير جديد - يتم تحليل البيانات'], charts: 2 };
  ...
  const getViewFields = (report: any): DetailField[] => [
    { label: 'العنوان', value: report.title }, { label: 'النوع', value: getTypeConfig(report.type).label },
    { label: 'الوصف', value: report.description }, { label: 'الحالة', value: report.status },
    { label: 'التكرار', value: report.frequency }, { label: 'آخر إنشاء', value: report.lastGenerated },
    { label: 'التنسيقات', value: report.format.join(', ') }, { label: 'أهم النتائج', value: report.insights.join(' | ') },
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="relative flex-1 lg:w-96"><Search className="absolute right-3 top-3 h-4 w-4 text-gray-400" /><Input placeholder="البحث في التقارير..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pr-10" /></div>
          ...
          <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg font-arabic bg-white">
            <option value="last7days">آخر 7 أيام</option><option value="last30days">آخر 30 يوم</option><option value="last3months">آخر 3 أشهر</option><option value="last6months">آخر 6 أشهر</option><option value="lastyear">السنة الماضية</option>
          </select>
        </div>
        <div className="flex gap-2">
          <BaseActionButton variant="outline" className="font-arabic" onClick={handleRefresh}><RefreshCw className="ml-2 h-4 w-4" />تحديث</BaseActionButton>
          <BaseActionButton onClick={() => setShowNewReportForm(!showNewReportForm)} className="bg-blue-600 hover:bg-blue-700 text-white font-arabic"><Plus className="ml-2 h-4 w-4" />تقرير مخصص</BaseActionButton>
        </div>
      </div>

      {showNewReportForm && (
        <DataCardFrame title="إنشاء تقرير مخصص" icon={<Plus className="h-5 w-5" />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div><label className="block text-sm font-semibold font-arabic mb-2">عنوان التقرير *</label><Input placeholder="اسم وصفي للتقرير" value={newReport.title} onChange={e => setNewReport(p => ({ ...p, title: e.target.value }))} /></div>
            <div><label className="block text-sm font-semibold font-arabic mb-2">نوع التقرير</label><select value={newReport.type} onChange={e => setNewReport(p => ({ ...p, type: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg font-arabic bg-white">{reportTypes.map(type => (<option key={type.value} value={type.value}>{type.label}</option>))}</select></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div><label className="block text-sm font-semibold font-arabic mb-2">الفترة الزمنية</label><select value={newReport.period} onChange={e => setNewReport(p => ({ ...p, period: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg font-arabic bg-white"><option value="last30days">آخر 30 يوم</option><option value="last3months">آخر 3 أشهر</option><option value="last6months">آخر 6 أشهر</option><option value="lastyear">السنة الماضية</option></select></div>
            <div><label className="block text-sm font-semibold font-arabic mb-2">تنسيق التصدير</label><select value={newReport.format} onChange={e => setNewReport(p => ({ ...p, format: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg font-arabic bg-white"><option value="pdf">PDF</option><option value="excel">Excel</option><option value="word">Word</option></select></div>
          </div>
          <div className="flex gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <NumericStatCard title="إجمالي التقارير" value={reports.length} icon={<FileText className="h-5 w-5" />} accentColor="#3B82F6" />
        <NumericStatCard title="تقارير جاهزة" value={reports.filter(r => r.status === 'جاهز').length} icon={<Calendar className="h-5 w-5" />} accentColor="#10B981" />
        <NumericStatCard title="تقارير دورية" value={reports.filter(r => r.frequency === 'شهري').length} icon={<RefreshCw className="h-5 w-5" />} accentColor="#F97316" />
        <NumericStatCard title="إجمالي الرسوم البيانية" value={reports.reduce((sum, r) => sum + r.charts, 0)} icon={<BarChart3 className="h-5 w-5" />} accentColor="#8B5CF6" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredReports.map((report) => {
          ...
          return (
            <DataCardFrame key={report.id} title={report.title} icon={<IconComponent className="h-5 w-5" />} className="hover:shadow-lg transition-shadow">
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-1">
                  {report.insights.map((insight, index) => (
                    <li key={index} className="text-sm text-gray-600 font-arabic flex items-center"><div className="h-1 w-1 bg-blue-500 rounded-full ml-2" />{insight}</li>
                  ))}
                </ul>
              </div>
              <div className="flex gap-2">
                <BaseActionButton size="sm" variant="outline" className="flex-1 font-arabic" onClick={() => setViewingReport(report)}><Eye className="h-3 w-3 ml-1" />عرض</BaseActionButton>
                <BaseActionButton size="sm" variant="outline" className="font-arabic" onClick={() => handleDownloadReport(report)}><Download className="h-3 w-3 ml-1" />تحميل</BaseActionButton>
                <BaseActionButton size="sm" variant="outline" className="font-arabic" onClick={() => handleShareReport(report)}><Share className="h-3 w-3 ml-1" />مشاركة</BaseActionButton>
                {report.status === 'جاهز' && (
                  <BaseActionButton size="sm" className="bg-blue-600 hover:bg-blue-700 text-white font-arabic" onClick={() => handleRefreshReport(report.id)}><RefreshCw className="h-3 w-3 ml-1" />تحديث</BaseActionButton>
                )}
              </div>
            </DataCardFrame>
          );
      ...
      <DataCardFrame title="مولد التقارير بالذكاء الاصطناعي" icon={<BarChart3 className="h-5 w-5" />}>
