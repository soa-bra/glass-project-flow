...
const ProjectsList = ({
  ...
  return (
    <div className={`
      h-full bg-soabra-projects-bg rounded-t-2xl shadow-lg border border-gray-100 overflow-hidden z-projects 
      transition-opacity duration-300 glass
      ${isCompressed ? 'opacity-50' : 'opacity-100'}
    `}>
      <div className="h-full flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-gray-200 bg-white/60 backdrop-blur-sm rounded-t-2xl">
            ...
            {projects.map(project => {
              ...
              return (
                <div
                  ...
                  className="h-[140px] w-[90%] mx-auto bg-soabra-card-bg project-card rounded-xl p-4 cursor-pointer hover:opacity-90 hover:shadow-lg transition-all duration-300 border border-white/20 backdrop-blur-sm glass-hover"
                >
                  {/* First Row */}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="h-10 px-4 bg-white/40 rounded-full 
                                  flex items-center justify-center backdrop-blur-sm border border-white/30">
                      <span className="text-sm font-medium text-soabra-text-primary truncate">
                        {project.assignee}
                      </span>
                    </div>

                    {/* Budget Bubble */}
                    <div className="h-10 px-3 bg-white/40 rounded-full 
                                  flex items-center justify-center backdrop-blur-sm border border-white/30">
                      <span className="text-sm font-bold text-soabra-text-primary">
                        S.R {parseInt(project.value).toLocaleString()}
                      </span>
                    </div>
