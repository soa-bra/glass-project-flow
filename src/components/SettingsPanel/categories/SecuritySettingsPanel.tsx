...
export const SecuritySettingsPanel: React.FC<SecuritySettingsPanelProps> = () => {
  const [formData, setFormData] = useState({
    ...
    apiKeys: [{
      id: '1',
      name: 'API Key للتطبيق الرئيسي',
      created: '2024-01-15',
      lastUsed: '2024-01-20'
    }, {
      id: '2',
      name: 'API Key للتكاملات',
      created: '2024-01-10',
      lastUsed: '2024-01-19'
    }],
    trustedDevices: [{
  ...
  const [securityAlerts, setSecurityAlerts] = useState([{
    id: '1',
    type: 'suspicious',
    message: 'محاولة دخول مشبوهة من عنوان IP غير معروف',
    severity: 'high',
    time: '2024-01-20 08:30'
  }, {
    id: '2',
    type: 'policy',
    message: 'يُنصح بتفعيل MFA للأجهزة الجديدة',
    severity: 'medium',
  ...
  return <div className="h-full flex flex-col bg-transparent">
      ...
      <div className="flex-1 overflow-auto pb-6 px-0 my-[25px]">
        ...
        {/* Security Disclaimer */}
        <SecurityDisclaimer type="frontend-only" className="mb-4" />
      ...
      <div style={{
        ...
        <h3 className="text-md font-bold text-black mb-4 flex items-center gap-2">
          ...
          <span className="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs">AI Anomaly Model</span>
        </h3>
        <div className="space-y-3">
          {securityAlerts.map(alert => <div key={alert.id} className="rounded-lg p-3 flex items-center gap-3 bg-transparent">
      ...
      <div style={{
        ...
        <SecurityDisclaimer type="mock" feature="المصادقة الثنائية" className="mb-4" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div style={{
            ...
            <button onClick={() => setFormData(prev => ({
                ...
                mfa: {
                  ...prev.mfa,
                  enabled: !prev.mfa.enabled
                }
              }))} className={`w-full px-4 py-2 rounded-full text-sm font-medium ${formData.mfa.enabled ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-green-500 text-white hover:bg-green-600'}`}>
          ...
          <div style={{
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.mfa.methods.includes('totp')} onChange={e => {
                    const methods = e.target.checked ? [...formData.mfa.methods, 'totp'] : formData.mfa.methods.filter(m => m !== 'totp');
                    setFormData(prev => ({
                      ...prev,
                      mfa: {
                        ...prev.mfa,
              ...
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formData.mfa.methods.includes('sms')} onChange={e => {
                    const methods = e.target.checked ? [...formData.mfa.methods, 'sms'] : formData.mfa.methods.filter(m => m !== 'sms');
                    setFormData(prev => ({
                      ...prev,
                      mfa: {
                        ...prev.mfa,
      ...
      <div style={{
        ...
        <SecurityDisclaimer type="demo" feature="مفاتيح API" className="mb-4" />
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-md font-bold text-black">مفاتيح API</h3>
          <button onClick={generateNewApiKey} className="px-4 py-2 bg-black text-white rounded-full text-sm font-medium">
            إنشاء مفتاح جديد
          </button>
        </div>
        
        <div className="space-y-3">
          {formData.apiKeys.map(key => <div key={key.id} style={{
              backgroundColor: '#f2ffff'
              ...
              <button onClick={() => revokeApiKey(key.id)} className="px-3 py-1 bg- rounded-full font-medium text-[soabra-new-urgent-important] text-white bg-[soabra-new-urgent-important] bg-rose-600 hover:bg-rose-500">
                إلغاء
              </button>
            </div>)}
        </div>
      ...
      <div style={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {formData.trustedDevices.map(device => <div key={device.id} style={{
              backgroundColor: '#f2ffff'
      ...
      <div style={{
        ...
        <div className="space-y-3">
          {formData.activeSessions.map(session => <div key={session.id} style={{
              backgroundColor: '#f2ffff'
            }} className="rounded-lg p-4 flex items-center justify-between">
              <div className="flex-1">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => terminateSession(session.id)} className="px-3 py-1 text-white rounded-full text-xs font-medium bg-rose-600 hover:bg-rose-500">
                  إنهاء الجلسة
                </button>
              </div>
            </div>)}
