...
export const IntegrationsSettingsPanel: React.FC<IntegrationsSettingsPanelProps> = () => {
  ...
  return <div className="h-full flex flex-col bg-transparent">
      ...
      <div style={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {integrations.map(integration => {
              ...
              return <div key={integration.id} style={{
                backgroundColor: '#f2ffff'
                ...
                <div className="space-y-2">
                  ...
                  {integration.lastSync && <div className="flex items-center justify-between text-xs">
                      <span className="text-gray-600">آخر مزامنة:</span>
                      <span className="font-medium text-black">{integration.lastSync}</span>
                    </div>}
                  
                  {integration.apiKey && <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex gap-2 mt-4">
                  {integration.status === 'connected' ? <>
                      <button onClick={() => disconnectIntegration(integration.id)} className="flex-1 px-3 py-2 bg-red-500 text-white rounded-full text-xs font-medium">
                        قطع الاتصال
                      </button>
                      ...
                      <button onClick={() => startAISetupAssistant(integration.id)} className="flex-1 px-3 py-2 bg-[soabra-new-project-summary] text-white rounded-full text-xs font-medium "> إعداد ذكي</button>
                      <button onClick={() => setShowSetupWizard(integration.id)} className="flex-1 px-3 py-2  bg-[soabra-new-secondary-1] text-white rounded-full text-xs font-medium">
                        إعداد يدوي
                      </button>
                    </>}
                </div>
      ...
      {showSetupWizard && <div style={{
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2">
              {[1, 2, 3].map(step => <div key={step} className="flex items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step <= setupStep ? 'bg-black text-white' : 'bg-gray-200 text-gray-600'}`}>
          ...
          {setupStep === 1 && <div className="space-y-4">
              <h4 className="text-sm font-bold text-black">الخطوة 1: معلومات الاتصال</h4>
              <div className="space-y-3">
                <input type="text" placeholder="API Key" value={setupData.suggestedConfig?.apiKey || ''} className="w-full p-3 rounded-lg border text-sm" onChange={e => setSetupData(prev => ({
                ...prev,
                suggestedConfig: {
                  ...prev.suggestedConfig,
                  apiKey: e.target.value
                }
              }))} />
                <input type="url" placeholder="Webhook URL" value={setupData.suggestedConfig?.webhook || ''} className="w-full p-3 rounded-lg border text-sm" onChange={e => setSetupData(prev => ({
                ...prev,
                suggestedConfig: {
                  ...prev.suggestedConfig,
          ...
          {setupStep === 2 && <div className="space-y-4">
              <h4 className="text-sm font-bold text-black">الخطوة 2: الصلاحيات</h4>
              <div className="space-y-2">
                {['read', 'write', 'admin'].map(scope => <label key={scope} className="flex items-center gap-2">
          ...
          {setupStep === 3 && <div className="space-y-4">
          ...
          <div className="flex gap-3 mt-6">
            {setupStep > 1 && <button onClick={() => setSetupStep(setupStep - 1)} className="px-4 py-2 bg-gray-200 text-black rounded-full text-sm font-medium">
                السابق
              </button>}
            
            {setupStep < 3 ? <button onClick={() => setSetupStep(setupStep + 1)} className="px-4 py-2 bg-black text-white rounded-full text-sm font-medium">
                التالي
              </button> : <button onClick={() => connectIntegration(showSetupWizard)} className="px-4 py-2 bg-green-500 text-white rounded-full text-sm font-medium">
                إنهاء الإعداد
              </button>}
      ...
      <div className="grid grid-cols-4 gap-4">
        <div style={{
            backgroundColor: '#f2ffff'
          }} className="rounded-2xl p-4 border border-black/10 text-center">
