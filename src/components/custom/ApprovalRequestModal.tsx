    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-black/10 p-8 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-black">طلب موافقة مالية</h2>
          <button 
            onClick={handleClose}
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-transparent border rounded-full text-black placeholder-black/50 focus:outline-none focus:ring-2 focus:ring-black/20 ${
                errors.requiredBudget ? 'border-red-500' : 'border-black/20'
              }`}
            />
            {errors.requiredBudget && (
              <p className="text-red-500 text-xs mt-1">{errors.requiredBudget}</p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="اكتب مبررات طلب الميزانية الإضافية..."
              rows={4}
              className={`w-full px-4 py-3 bg-transparent border rounded-2xl text-black placeholder-black/50 focus:outline-none focus:ring-2 focus:ring-black/20 resize-none ${
                errors.justification ? 'border-red-500' : 'border-black/20'
              }`}
            />
            ...
            {attachments.length > 0 && (
              <div className="mt-2 space-y-1">
                {attachments.map((file, index) => (
                  <div key={index} className="text-xs text-black/70 bg-black/5 px-2 py-1 rounded-full">
                    {file.name}
                  </div>
                ))}
              </div>
        ...
        <div className="flex gap-3 mt-8">
          <button
            onClick={handleClose}
            className="flex-1 px-4 py-3 bg-transparent border border-black/20 text-black rounded-full text-sm hover:bg-black/5 transition-colors"
          >
            إلغاء
          </button>
          <button
            onClick={handleSave}
            className="flex-1 px-4 py-3 bg-black text-white rounded-full text-sm hover:bg-black/80 transition-colors"
          >
            إرسال الطلب
          </button>
        </div>
