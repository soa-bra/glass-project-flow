...
interface ManualTaskDistributionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (redistributedTasks: { taskId: string; assigneeId: string }[]) => void;
}
...
export const ManualTaskDistributionModal: React.FC<ManualTaskDistributionModalProps> = ({
  ...
  const handleTaskAssigneeChange = (taskId: string, newAssigneeId: string) => {
    setTasks(prevTasks => 
      prevTasks.map(task => 
        task.id === taskId 
          ? { ...task, currentAssignee: newAssigneeId }
          : task
      )
    );
  };
  ...
  const handleSmartDistribution = async () => {
    ...
    const redistributedTasks = tasks.map(task => {
      ...
      if (suitableMembers.length > 0) {
        ...
        return { ...task, currentAssignee: bestMember.id };
      }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-black/10 w-full max-w-6xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-black/10">
          <h2 className="text-xl font-bold text-black">توزيع المهام يدوياً أو بالذكاء الاصطناعي</h2>
          <button
            onClick={onClose}
          ...
          <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl border border-black/10 p-4 mb-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={isSmartDistributing}
                className="px-6 py-3 bg-black text-white rounded-full text-sm hover:bg-black/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {isSmartDistributing ? (
                  <>
                  ...
                  <div key={member.id} className="bg-white/50 rounded-2xl border border-black/10 p-4">
                    ...
                    <div className="w-full bg-black/10 rounded-full h-2 mb-2">
                      <div 
                        ...
                        style={{ width: `${Math.min((member.assignedHours / member.maxCapacity) * 100, 100)}%` }}
                      />
                    </div>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <div className="mb-2">
                          ...
                          <select
                            value={task.currentAssignee || ''}
                            onChange={(e) => handleTaskAssigneeChange(task.id, e.target.value)}
                            className="w-full p-2 bg-white/50 border border-black/20 rounded-xl text-sm text-black focus:outline-none focus:ring-2 focus:ring-black/20"
                          >
