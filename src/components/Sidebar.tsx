...
const Sidebar = ({
  ...
  }} className="bg-soabra-solid-bg z-sidebar h-full backdrop-blur-xl rounded-3xl mx-0 overflow-hidden px-0 bg-[dfecf2]">
      <nav className="flex flex-col gap-2 h-full py-0 mx-0 px-0 bg-[dfecf2]">
        ...
        <div className={`text-center mb-2 rounded-full mx-0 px-0 py-[24px] my-[24px] sync-transition ${isCollapsed ? 'flex justify-center' : ''}`}>
          <div className={`flex items-center rounded-lg sync-transition ${isCollapsed ? 'justify-center px-0 mx-0' : 'justify-between px-[3px] mx-[20px]'}`}>
            ...
            <button onClick={toggleSidebar} className="group w-[60px] h-[60px] rounded-full flex items-center justify-center border-2 border-[#3e494c]/30 hover:border-[#3e494c]/60 hover:bg-white/20 hover:shadow-lg flex-shrink-0 sync-transition-fast" style={{
            transform: 'scale(1)',
            transition: 'all var(--animation-duration-fast) var(--animation-easing)'
          }}>
              <div className="sync-transition-fast" style={{
        ...
        <div className={`flex flex-col gap-2 px-0 sync-transition ${isCollapsed ? 'mx-[15px]' : 'mx-[15px]'}`}>
          ...
          return <a key={index} href={item.link} className={`
                  flex items-center gap-3 text-right group relative overflow-hidden sync-transition
                  ${item.active ? 'bg-white/25 text-[#3e494c] font-medium rounded-full shadow-lg border border-white/50 scale-[1.02]' : 'text-soabra-text-secondary hover:bg-white/15 hover:text-[#3e494c] font-light rounded-full hover:shadow-md border border-transparent hover:border-white/30 hover:scale-[1.02] active:scale-95'}
            ...
            transitionDelay: isCollapsed ? '0ms' : `calc(var(--stagger-delay) * ${index})`
                ...
                <div className={`
                    ...
                    ${item.active ? 'bg-white/30 border-[#3e494c]/60 shadow-lg scale-105' : 'border-[#3e494c]/30 group-hover:border-[#3e494c]/60 group-hover:bg-white/20 group-hover:shadow-md group-hover:scale-110 group-active:scale-95'}
                  `}>
                  <IconComponent className={`
                      w-6 h-6 sync-transition-fast
                      ${item.active ? 'text-[#3e494c] scale-110' : 'text-soabra-text-secondary group-hover:text-[#3e494c] group-hover:scale-110'}
                    `} />
                </div>
