...
const Sidebar = ({ onToggle }: SidebarProps) => {
  ...
  return (
    <aside 
      className="bg-soabra-solid-bg z-sidebar h-full backdrop-blur-xl rounded-3xl transition-all duration-500 ease-in-out"
      style={{
        width: isCollapsed ? 'var(--sidebar-width-collapsed)' : 'var(--sidebar-width-expanded)'
      }}
    >
      <nav className="flex flex-col gap-2 my-0 py-[10px] px-0 mx-[var(--sidebar-margin)]">
        ...
        <div className="text-center mb-2 rounded-full py-[31px] px-0 my-[34px]">
          <div className={`flex items-center rounded-lg px-[3px] mx-[5px] my-0 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            ...
            <button
              ...
              className="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ease-out border-2 border-[#3e494c]/30 hover:border-[#3e494c]/60 hover:bg-white/10 hover:scale-105 active:scale-95 text-3xl mx-0 px-[15px]"
            >
