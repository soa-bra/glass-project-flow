...
const Sidebar = ({
  ...
  return (
    <aside 
      ...
      className="bg-soabra-solid-bg z-sidebar h-full backdrop-blur-xl rounded-3xl mx-0 px-px overflow-hidden"
    >
      <nav className="flex flex-col gap-2 h-full py-0 px-0">
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span 
                    className={`tracking-wide text-base whitespace-nowrap sync-transition-fast ${item.active ? 'font-semibold' : 'group-hover:font-medium'}`}
                  >
                    {item.label}
                  </span>
                </div>
