  ...
  return (
    <aside className={`bg-soabra-solid-bg z-sidebar h-full backdrop-blur-xl rounded-3xl transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-60'}`}>
      <nav className="flex flex-col gap-2 py-4 px-3">
        ...
        <div className="text-center mb-2 rounded-full py-1 px-2">
          <div className="flex items-center justify-between">
            {!isCollapsed && (
              <h2 className="text-soabra-text-primary text-right text-xl font-medium flex-1 px-3">
                القائمة
              </h2>
            )}
            <button 
              onClick={toggleSidebar} 
              className="w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 border-2 border-[#3e494c]/50 hover:bg-white/10"
            >
              {isCollapsed ? (
