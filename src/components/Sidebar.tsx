...
const Sidebar = () => {
  ...
  return <aside className={`bg-soabra-solid-bg z-sidebar h-full backdrop-blur-xl rounded-3xl transition-all duration-500 ease-in-out ${isCollapsed ? 'w-20' : 'w-60'}`}>
      <nav className="flex flex-col gap-2 py-0 my-0 px-0 mx-0">
        ...
        <div className="text-center mb-2 rounded-full px-[15px] py-[31px] my-[46px]">
        ...
        return <button key={index} className={`
            ...
            transitionDelay: `${baseDelay}ms`
                ...
                {!isCollapsed && <>
                    ...
                    {item.active && <div className={`w-2 h-2 bg-[#3e494c] rounded-full ml-2 mx-[70px] px-0 transition-all duration-400 ease-in-out group-hover:scale-110 ${isCollapsed ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`} style={{
                transitionDelay: isCollapsed ? '0ms' : `${baseDelay + 200}ms`
              }} />}
                  </>}
              </div>
            </button>;
