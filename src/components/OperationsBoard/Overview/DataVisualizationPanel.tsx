...
export const DataVisualizationPanel: React.FC<DataVisualizationPanelProps> = ({
  ...
  const renderChart = () => {
    if (chart === 'bar') {
      return <div className="flex items-end justify-center gap-1 h-16 mb-4">
          {[40, 30, 50, 20, 35].map((height, index) => <div key={index} className="rounded-sm transition-all duration-300" style={{
          width: '12px',
          height: `${height}px`,
      ...
      return <div className="flex items-center justify-center mb-4">
          <div className="relative w-20 h-20">
            <svg className="w-20 h-20 transform -rotate-90">
              ...
              <circle cx="40" cy="40" r="32" stroke={COLORS.primary} strokeWidth="6" fill="transparent" strokeDasharray={`${75 / 100 * 201} 201`} className="transition-all duration-300" />
            </svg>
      ...
      return <div className="h-16 relative mb-4">
          <svg className="w-full h-full">
            <path d="M 0 40 Q 20 20 40 35 T 80 30 T 120 25" stroke={COLORS.secondary_4} strokeWidth="3" fill="none" className="opacity-80" />
            {/* نقاط البيانات */}
