...
export const AlertsBox: React.FC = () => {
  return (
    <BaseBox variant="standard" size="md" className="h-full min-h-0 flex flex-col" overflow="hidden">
            ...
            {alerts.map(alert => (
              <div key={alert.id} className="flex items-center justify-between py-3 px-3 rounded-lg border border-[#DADCE0] bg-white">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[hsl(var(--ink))] font-arabic truncate">
                    {alert.message}
                  </p>
                ...
                <div className={`w-3 h-3 rounded-full ms-3 flex-shrink-0 ${
