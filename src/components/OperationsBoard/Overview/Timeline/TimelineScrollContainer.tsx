...
export const TimelineScrollContainer: React.FC<TimelineScrollContainerProps> = ({
  ...
  return <div className="h-full relative overflow-hidden rounded-2xl px-0 my-0 py-[9px]">
      {/* مؤشر التمرير الأيسر */}
      ...
      WebkitOverflowScrolling: 'touch'
        ...
        <div className="flex items-center gap-20 min-w-max relative z-20 py-0 px-[25px]">
          {timeline.map(event => <TimelineEventItem key={event.id} event={event} onEventClick={onEventClick} />)}
        </div>
      </div>
    </div>;
};