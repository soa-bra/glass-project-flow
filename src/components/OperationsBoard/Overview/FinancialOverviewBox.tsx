      ...
      header={
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="w-8 h-8 bg-transparent border border-[hsl(var(--ink))]/20 rounded-full flex items-center justify-center hover:bg-[hsl(var(--ink))]/10 transition-colors"
          >
      ...
      <div className="flex gap-4 h-full overflow-hidden">
        <div className="w-2/5 flex flex-col justify-center overflow-hidden">
          <div className="text-center mb-4">
            <div className="text-2xl font-bold text-[hsl(var(--ink))] font-arabic mb-1">{totalRevenue}</div>
            <div className="text-xs font-normal text-[hsl(var(--ink))] font-arabic text-center">إجمالي الأرباح والخسائر</div>
          </div>
          <div className="grid grid-cols-1 gap-2 text-center mb-4">
            <div>
              <div className="text-2xl font-bold text-[hsl(var(--ink))] font-arabic">02</div>
              <div className="text-xs font-normal text-[hsl(var(--ink-60))] font-arabic">مثال</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-[hsl(var(--ink))] font-arabic">14</div>
              <div className="text-xs font-normal text-[hsl(var(--ink-60))] font-arabic">مثال</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-[hsl(var(--ink))] font-arabic">78</div>
              <div className="text-xs font-normal text-[hsl(var(--ink-60))] font-arabic">مثال</div>
            </div>
          </div>
          <div className="text-xs font-normal text-[hsl(var(--ink))] font-arabic text-center mb-2">هذا النص مثال للشكل النهائي</div>
          <div className="text-xs font-normal text-[hsl(var(--ink))] font-arabic text-center">هذا النص مثال</div>
        </div>

        <div className="w-3/5 flex justify-center items-center overflow-hidden">
          <div className="relative w-48 h-48">
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-4xl font-bold text-[hsl(var(--ink))] font-arabic mb-1">{totalRevenue}</div>
              <div className="text-xs font-normal text-[hsl(var(--ink))] font-arabic text-center px-2">إجمالي الأرباح والخسائر</div>
            </div>
            <svg className="w-full h-full" viewBox="0 0 200 200">
              {circularBars.map((bar) => {
                ...
                return (
                  <line key={bar.id} x1={x1} y1={y1} x2={x2} y2={y2} stroke={bar.color} strokeWidth={barWidth} strokeLinecap="round" />
                );
              })}
            </svg>
          </div>
