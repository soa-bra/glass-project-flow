  ...
  return (
    <div className="relative flex flex-col items-center min-w-fit group px-8">
      ...
      <div className="text-center mb-6 whitespace-nowrap">
        <div className="text-sm font-medium text-gray-600 font-arabic mb-1">
          {new Date(event.date).toLocaleDateString('ar-EG', {
            month: 'short'
          })}
        </div>
        <div className="text-3xl font-bold text-gray-900 font-arabic">
          {new Date(event.date).getDate().toString().padStart(2, '0')}
        </div>
      </div>

      {/* النقطة البيضاء على الخط */}
      <button 
        className="
          w-4 h-4 rounded-full bg-white border-2 border-gray-400 
      ...
      <div className="text-center mt-6 max-w-44 transition-all duration-200 group-hover:scale-105">
