...
export const ContractForm: React.FC<ContractFormProps> = ({
  ...
  return <div className="space-y-6">
      ...
      WebkitBackdropFilter: 'blur(10px)'
        ...
        }} className={`text-right font-arabic ${validationErrors.contractValue ? 'border-red-500' : ''}`} placeholder="0" />
              ...
              {projectData.contractPayments.map((payment: ContractPayment) => (
                <div key={payment.id} className="grid grid-cols-3 gap-4 p-3 bg-white/10 rounded-lg">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="date" value={payment.date} onChange={e => {
                onUpdatePayment(payment.id, 'date', e.target.value);
                validateField(`paymentDate_${payment.id}`, e.target.value);
              }} className={`text-right font-arabic text-sm ${validationErrors[`paymentDate_${payment.id}`] ? 'border-red-500' : ''}`} />
