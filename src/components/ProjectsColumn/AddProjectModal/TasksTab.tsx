  ...
  return <div className="space-y-6">
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <Button onClick={onAddTask} className="bg-black text-white hover:bg-gray-800 font-arabic rounded-full">
            إضافة مهمة +
          </Button>
          <h3 className="text-lg font-bold font-arabic">مهام المشروع</h3>
        </div>
        
        {tasks.length === 0 ? <div className="text-center py-8 text-gray-500 font-arabic">
            لا توجد مهام مضافة بعد
          </div> : <div className="space-y-3">
            {tasks.map((task, index) => <div key={index} className="p-4 border border-white/40 rounded-lg" style={{
