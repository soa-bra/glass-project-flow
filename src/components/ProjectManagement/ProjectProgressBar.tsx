...
export const ProjectProgressBar: React.FC<ProjectProgressBarProps> = ({
  ...
  return (
    <div className="w-full flex justify-center font-arabic">
      {/* الحاوية الرئيسية - مركزة و 80% من العرض */}
      <div className="relative w-[80%] flex flex-col items-start pt-12 pb-10" style={{ background: 'transparent' }}>
        {/* فقاعة تقدم المشروع - ثابتة في الجهة اليسرى */}
        <div
          className="absolute z-10 text-black text-right"
          style={{
            background: '#B9F3A8', // اللون الأخضر الصلب المطلوب
            boxShadow: '0 2px 8px rgba(0,0,0,0.1)',
            padding: '14px 20px',
            borderRadius: '50px',
            minWidth: '160px',
            top: `-${circleSize / 1.8}px`,
            left: '0px' // ثابتة في الجهة اليسرى
          }}
        >
        ...
        {/* شريط الشرائح */}
        <div ref={barRef} className="relative flex items-center justify-start flex-row-reverse gap-[3px] w-full h-[25px]">
          {segments.map((idx) => {
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    backgroundColor: isCompleted ? '#B9F3A8' : 'rgba(255, 255, 255, 0.3)', // اللون الأخضر الصلب المطلوب
                    backdropFilter: 'blur(16px)',
                    border: 'none', // إزالة الحد الأسود
                    boxShadow: isCompleted
                      ? '0 0 6px #B9F3A8' // ظل بنفس اللون الأخضر
