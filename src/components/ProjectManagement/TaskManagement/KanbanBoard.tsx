...
const columns: KanbanColumn[] = [{
  name: "To-Do",
  color: COLORS.TASK_STATUS_TODO,
  description: "المهام المجدولة",
  status: "todo"
}, {
  name: "Stopped",
  color: COLORS.TASK_STATUS_STOPPED,
  description: "المهام المتوقفة",
  status: "stopped"
}, {
  name: "Treating",
  color: COLORS.TASK_STATUS_TREATING,
  description: "المهام تحت المعالجة",
  status: "treating"
}, {
  name: "Late",
  color: COLORS.TASK_STATUS_LATE,
  description: "المهام المتأخرة",
  status: "late"
}, {
  name: "In Progress",
  color: COLORS.TASK_STATUS_IN_PROGRESS,
  description: "المهام الجارية",
  status: "in-progress"
}, {
  name: "Done",
  color: COLORS.TASK_STATUS_COMPLETED,
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  return <div className="space-y-4">
      {/* Bulk Actions Bar */}
      {selectedTasks.length > 0 && <BulkActionsBar selectedCount={selectedTasks.length} onClearSelection={() => setSelectedTasks([])} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-6 gap-4 min-h-[600px]">
        ...
        return <div key={column.name} onDragOver={handleDragOver} onDrop={e => handleDrop(e, column.status)} className="ounded-[41px] bg-[#FFFFFF] border border-[#DADCE0] flex flex-col">
              {/* Column Header */}
              <div className="p-4 rounded-t-3xl border-b border-black/10" style={{
            backgroundColor: column.color
              ...
              return <div key={task.id} draggable onDragStart={() => handleDragStart(task, column.status)}>
                      <TaskCard {...taskCardProps} isSelected={selectedTasks.includes(task.id)} isSelectionMode={false} onSelect={() => handleTaskSelect(task.id)} />
                    </div>;
            })}
