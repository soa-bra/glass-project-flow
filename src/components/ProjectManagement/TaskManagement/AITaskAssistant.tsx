...
export const AITaskAssistant: React.FC<AITaskAssistantProps> = ({
  ...
  return <div className="ounded-[41px] bg-[#FFFFFF] border border-[#DADCE0]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        {aiFeatures.map(feature => <div key={feature.id} className={`p-4 bg-transparent border border-black/10 rounded-3xl cursor-pointer transition-all duration-200 hover:shadow-lg ${activeFeature === feature.id ? 'ring-2 ring-black' : ''}`} onClick={() => setActiveFeature(activeFeature === feature.id ? null : feature.id)}>
      ...
      <div className="space-y-4">
        ...
        {suggestions.map(suggestion => <div key={suggestion.id} className="flex items-center justify-between p-4 bg-transparent border border-black/10 rounded-full">
