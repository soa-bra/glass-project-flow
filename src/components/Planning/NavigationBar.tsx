...
const NavigationBar: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 flex items-center gap-2 bg-white rounded-[18px] px-4 py-3 shadow-[0_1px_1px_rgba(0,0,0,0.04),0_12px_28px_rgba(0,0,0,0.10)] border border-sb-border">
      {/* Zoom Out */}
      <button
        onClick={zoomOut}
        ...
        title="تصغير (Ctrl + -)"
      >
        <ZoomOut size={18} className="text-sb-ink" />
      </button>
      ...
      <input
        ...
        className="w-32 h-2 bg-sb-panel-bg rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-sb-ink"
      ...
      <button
        ...
        title="تكبير (Ctrl + +)"
      >
        <ZoomIn size={18} className="text-sb-ink" />
      </button>
      ...
      <div className="relative">
        <button
          ...
          className="px-3 py-1 hover:bg-sb-panel-bg rounded-lg transition-colors text-[13px] font-medium text-sb-ink min-w-[64px]"
        >
          {zoomPercentage}%
        </button>
      ...
      <div className="h-6 w-px bg-sb-border mx-1" />
      ...
      >
        <Maximize size={18} className="text-sb-ink" />
      </button>
