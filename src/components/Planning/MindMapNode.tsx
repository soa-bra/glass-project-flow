  ...
  return <div ref={nodeRef} className={`absolute select-none transition-shadow ${activeTool === "selection_tool" ? "cursor-move" : "cursor-default"} ${isSelected ? "ring-2 ring-[hsl(var(--accent-green))] ring-offset-2" : ""}`} style={{
    ...
    zIndex: isSelected ? 100 : 10
      ...
      {isSelected && !isEditing && activeTool === 'smart_element_tool' && <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-white/95 backdrop-blur-md rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] p-1.5 border border-[hsl(var(--border))] z-[100]" onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
          {/* إضافة فرع */}
          <button onClick={handleAddBranch} className="p-2 rounded-lg hover:bg-[hsl(var(--muted))] text-[hsl(var(--ink-60))] hover:text-[hsl(var(--accent-green))] transition-colors" title="إضافة فرع">
            <Plus size={16} />
          </button>

          {/* تبديل شكل العقدة */}
          <div className="relative">
            <button onClick={() => setShowStylePicker(!showStylePicker)} className="p-2 rounded-lg hover:bg-[hsl(var(--muted))] text-[hsl(var(--ink-60))] hover:text-[hsl(var(--accent-blue))] transition-colors" title="شكل العقدة">
              {nodeData.nodeStyle === 'circle' ? <Circle size={16} /> : nodeData.nodeStyle === 'rectangle' ? <Square size={16} /> : nodeData.nodeStyle === 'pill' ? <Pill size={16} /> : <RectangleHorizontal size={16} />}
            </button>
          ...
          backgroundColor: nodeData.color || '#3DA8F5'
        }} title="تغيير اللون" className="w-4 h-4 hover:scale-110 transition-all border-2 border-white shadow-sm rounded-sm" />
          ...
          {/* حذف */}
          <button onClick={() => deleteElement(element.id)} className="p-2 rounded-lg hover:bg-red-50 text-[hsl(var(--ink-60))] hover:text-[#E5564D] transition-colors" title="حذف العقدة">
            <Trash2 size={16} />
          </button>
        </div>}
    </div>;
