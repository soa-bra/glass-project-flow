...
import { Brain, Calendar, CheckSquare, Vote, Lightbulb, Users, Target, Workflow, Clock, BarChart3, Search, Settings, Sparkles, Grid3X3, List, Filter, Star, Plus, Zap, Palette, Layout, Database, MessageCircle, FileText, PieChart, TrendingUp, GitBranch, MapPin, Heart, Award, Bookmark } from 'lucide-react';
...
export const EnhancedSmartElementsPanel: React.FC<EnhancedSmartElementsPanelProps> = ({
  ...
  const filteredElements = useMemo(() => {
    return smartElementsData.filter(element => {
      ...
      const matchesSearch = element.name.toLowerCase().includes(searchTerm.toLowerCase()) || element.description.toLowerCase().includes(searchTerm.toLowerCase()) || element.tags.some(tag => tag.includes(searchTerm));
  ...
  return <Card className="h-full flex flex-col w-60 backdrop-blur-sm border-border/50 mx-[3px] px-[5px]">
          ...
          <TabsContent value="browse" className="flex-1 m-0 flex flex-col">
            ...
            <div className="space-y-3 mb-4">
              <div className="relative">
                ...
                <Input placeholder="البحث في العناصر..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9 h-8 text-xs" />
              </div>

              <div className="flex items-center justify-between">
                <div className="flex gap-1">
                  <Button size="sm" variant={viewMode === 'grid' ? 'default' : 'outline'} onClick={() => setViewMode('grid')} className="h-7 w-7 p-0">
                    <Grid3X3 className="w-3 h-3" />
                  </Button>
                  <Button size="sm" variant={viewMode === 'list' ? 'default' : 'outline'} onClick={() => setViewMode('list')} className="h-7 w-7 p-0">
                    <List className="w-3 h-3" />
                  </Button>
                </div>

                <div className="flex items-center gap-2">
                  <Switch checked={showFeaturedOnly} onCheckedChange={setShowFeaturedOnly} className="scale-75" />
              ...
              <div className="flex gap-1 overflow-x-auto">
                ...
                return <Button key={category.id} size="sm" variant={selectedCategory === category.id ? 'default' : 'outline'} onClick={() => setSelectedCategory(category.id)} className="text-xs whitespace-nowrap">
            ...
            <ScrollArea className="flex-1">
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 gap-3' : 'space-y-2'}>
                ...
                return <Card key={element.id} className="cursor-pointer hover:shadow-md transition-all duration-200 border-border/50 hover:border-primary/50" onClick={() => handleElementSelect(element)}>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {element.featured && <Star className="w-3 h-3 text-yellow-500 fill-current" />}
                            </div>
              ...
              {filteredElements.length === 0 && <div className="text-center py-8">
          ...
          <TabsContent value="customize" className="flex-1 m-0 flex flex-col">
            {selectedElement ? <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <Input placeholder="العنوان" value={elementConfig.content.title} onChange={e => setElementConfig(prev => ({
                      ...prev,
                      content: {
                        ...prev.content,
                        ...
                        <Input placeholder="العنوان الفرعي (اختياري)" value={elementConfig.content.subtitle || ''} onChange={e => setElementConfig(prev => ({
                      ...prev,
                      content: {
                        ...prev.content,
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <Slider value={[elementConfig.size.width]} onValueChange={([value]) => setElementConfig(prev => ({
                        ...prev,
                        size: {
                          ...prev.size,
                          width: value
                        }
                      }))} max={800} min={200} step={20} className="mt-1" />
                        </div>
                        <div>
                          <Label className="text-xs text-muted-foreground">الارتفاع: {elementConfig.size.height}px</Label>
                          <Slider value={[elementConfig.size.height]} onValueChange={([value]) => setElementConfig(prev => ({
                        ...prev,
                        size: {
                          ...prev.size,
                          height: value
                        }
                      }))} max={600} min={150} step={20} className="mt-1" />
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-1">
                            {['default', 'minimal', 'modern', 'corporate'].map(theme => <Button key={theme} size="sm" variant={elementConfig.style.theme === theme ? 'default' : 'outline'} onClick={() => setElementConfig(prev => ({
                          ...prev,
                          style: {
                            ...prev.style,
                            theme: theme as any
                                ...
                                {theme === 'default' ? 'افتراضي' : theme === 'minimal' ? 'بسيط' : theme === 'modern' ? 'عصري' : 'مؤسسي'}
                              </Button>)}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="flex gap-1">
                            {['blue', 'green', 'purple', 'orange', 'red'].map(color => <Button key={color} size="sm" variant={elementConfig.style.colorScheme === color ? 'default' : 'outline'} onClick={() => setElementConfig(prev => ({
                          ...prev,
                          style: {
                            ...prev.style,
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Switch checked={elementConfig.behavior.interactive} onCheckedChange={checked => setElementConfig(prev => ({
                        ...prev,
                        behavior: {
                          ...prev.behavior,
                          interactive: checked
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Switch checked={elementConfig.behavior.animated} onCheckedChange={checked => setElementConfig(prev => ({
                        ...prev,
                        behavior: {
                          ...prev.behavior,
                          animated: checked
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Switch checked={elementConfig.behavior.collaborative} onCheckedChange={checked => setElementConfig(prev => ({
                        ...prev,
                        behavior: {
                          ...prev.behavior,
                ...
                <div className="flex gap-2">
                  <Button onClick={handleAddElement} className="flex-1 text-xs" size="sm">
