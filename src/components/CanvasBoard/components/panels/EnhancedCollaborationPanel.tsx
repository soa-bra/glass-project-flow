...
import { Users, MessageCircle, Mic, MicOff, Phone, PhoneOff, Send, Smile, Link2, Crown, UserPlus, Settings, Volume2, VolumeX } from 'lucide-react';
...
export const EnhancedCollaborationPanel: React.FC<EnhancedCollaborationPanelProps> = ({
   ...
   */
  const renderParticipants = () => <div className="space-y-3">
      {/* إحصائيات المشاركين */}
      ...
      {isHost && <div className="space-y-2">
          {!showInviteForm ? <Button variant="outline" size="sm" onClick={() => setShowInviteForm(true)} className="w-full text-xs h-8">
              <UserPlus className="h-3 w-3 mr-1" />
              دعوة مشارك
            </Button> : <div className="space-y-2 p-2 border rounded-md">
              <Input placeholder="البريد الإلكتروني" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} className="h-7 text-xs" />
              <div className="flex gap-1">
                <select value={inviteRole} onChange={e => setInviteRole(e.target.value as Participant['role'])} className="flex-1 text-xs p-1 border rounded">
                  <option value="guest">ضيف</option>
                  <option value="internal_user">عضو</option>
                </select>
                <Button size="sm" onClick={handleInvite} className="h-7 px-2 text-xs">
                  دعوة
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setShowInviteForm(false)} className="h-7 px-2 text-xs">
                  إلغاء
                </Button>
              </div>
            </div>}
      ...
      <ScrollArea className="max-h-32">
        <div className="space-y-2">
          {displayParticipants.map(participant => <div key={participant.id} className="flex items-center gap-2 p-2 rounded-md hover:bg-muted/50 transition-colors">
              <div className="relative">
                ...
                <div className={cn("absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-background", getStatusColor(participant.status))} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {participant.isTyping && <span className="text-xs text-muted-foreground animate-pulse">
                      يكتب...
                    </span>}
                </div>
              </div>

              {participant.role !== 'host' && isHost && <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => onUpdatePermissions?.(participant.id, 'internal_user')}>
   ...
   */
  const renderTextChat = () => <div className="flex flex-col h-full">
      {/* رسائل الدردشة */}
      <ScrollArea className="flex-1 mb-3">
        <div className="space-y-2">
          ...
          return <div key={message.id} className={cn("flex flex-col", isSystemMessage ? "items-center" : isCurrentUser ? "items-end" : "items-start")}>
                {isSystemMessage ? <div className="text-xs text-muted-foreground text-center p-1">
                    {message.content}
                  </div> : <>
                    <div className={cn("max-w-[80%] rounded-lg px-2 py-1 text-xs", isCurrentUser ? "bg-primary text-primary-foreground" : "bg-muted")}>
                      {!isCurrentUser && <div className="text-xs font-medium mb-1 opacity-70">
                          {sender?.name || 'مجهول'}
                        </div>}
      ...
      <div className="flex gap-2">
        <Input placeholder="اكتب رسالة..." value={messageInput} onChange={e => setMessageInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} className="flex-1 h-8 text-xs" />
        <Button size="sm" onClick={handleSendMessage} disabled={!messageInput.trim()} className="h-8 px-2">
   ...
   */
  const renderVoiceChat = () => <div className="space-y-3">
      {/* حالة المكالمة */}
      <div className="text-center">
        {voiceChat.isActive ? <div className="space-y-2">
      ...
      {/* أدوات التحكم */}
      <div className="flex gap-1">
        <Button variant={voiceChat.isActive ? "destructive" : "default"} size="sm" onClick={() => onVoiceControl?.(voiceChat.isActive ? 'end' : 'start')} className="flex-1 text-xs h-8">
          {voiceChat.isActive ? <>
        ...
        {voiceChat.isActive && <Button variant={voiceChat.isMuted ? "secondary" : "outline"} size="sm" onClick={() => onVoiceControl?.(voiceChat.isMuted ? 'unmute' : 'mute')} className="h-8 px-2">
            {voiceChat.isMuted ? <MicOff className="h-3 w-3" /> : <Mic className="h-3 w-3" />}
          </Button>}
      </div>

      {/* مشاركو المكالمة */}
      {voiceChat.isActive && voiceChat.participants.length > 0 && <div className="space-y-2">
          ...
          return <div key={participantId} className="flex items-center gap-2 text-xs">
                  ...
                  {voiceChat.isMuted ? <VolumeX className="h-3 w-3 text-muted-foreground" /> : <Volume2 className="h-3 w-3 text-green-500" />}
                </div>;
        })}
          </div>
        </div>}
