...
export const ElementStylePanel: React.FC<ElementStylePanelProps> = ({
  ...
  const colorPalette = ['#000000', '#ffffff', '#96d8d0', '#f1b5b9', '#a4e2f6', '#d1e1ea', '#e9eff4', '#bdeed3', '#d9d2fd', '#fbe2aa', '#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4', '#ffeaa7', '#dda0dd', '#98d8c8', '#f7dc6f', '#bb8fce', '#85c1e9'];
  const borderStyles = [{
    value: 'solid',
    label: 'مصمت'
  }, {
  ...
  if (!selectedElement) {
    return <Card className="w-full h-full bg-[#f2f9fb]/95 backdrop-blur-xl shadow-sm border border-white/20 rounded-[32px] overflow-hidden mx-[20px] px-0">
        <CardContent className="flex items-center justify-center h-full mx-0 px-0">
      ...
      <CardContent className="space-y-4 h-[calc(100%-4rem)] overflow-y-auto">
        ...
        <div>
          ...
          <div className="flex gap-2 items-center">
            <Input type="color" value={selectedColor} onChange={e => handleColorChange(e.target.value)} className="w-16 h-12 rounded-[12px] border-[#d1e1ea] cursor-pointer" />
            <Input type="text" value={selectedColor} onChange={e => handleColorChange(e.target.value)} placeholder="#000000" className="flex-1 font-arabic text-sm rounded-[12px] border-[#d1e1ea] text-black" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <Slider value={[borderWidth]} onValueChange={handleBorderWidthChange} max={20} min={0} step={1} className="w-full" />
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              ...
              <Input type="text" value={borderColor} onChange={e => handleBorderColorChange(e.target.value)} className="flex-1 text-xs rounded-[8px] border-[#d1e1ea] text-black" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {borderStyles.map(style => <Button key={style.value} onClick={() => handleBorderStyleChange(style.value)} variant={borderStyle === style.value ? "default" : "outline"} size="sm" className={`rounded-[12px] text-xs font-arabic ${borderStyle === style.value ? 'bg-[#96d8d0] text-black hover:bg-[#96d8d0]/80' : 'border-[#d1e1ea] text-black hover:bg-[#e9eff4]/50'}`}>
                  {style.label}
                </Button>)}
            </div>
          </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Button size="sm" className="flex-1 rounded-[12px] bg-[#a4e2f6] hover:bg-[#a4e2f6]/80 text-black border-none">
              <Square className="w-4 h-4 mr-2" />
              مربع
            </Button>
            <Button size="sm" className="flex-1 rounded-[12px] bg-[#bdeed3] hover:bg-[#bdeed3]/80 text-black border-none">
