          ...
          <Tooltip key={tool.id}>
            <TooltipTrigger asChild>
              <Button
                ...
                size="sm"
                className={`h-12 w-12 rounded-xl transition-all duration-200 ${isSelected ? 'bg-black text-white shadow-lg' : 'text-gray-600 hover:bg-gray-100 hover:text-black'}`}
      ...
      <div className="flex items-center justify-1/2 gap-1">
        <Card className="bg-white/95 backdrop-blur-lg shadow-sm border border-gray-200/50 rounded-[24px] my-[20px]">
          <CardContent className="flex items-center justify-center gap-4 px-4 py-2">
            ...
            {basicTools.length > 0 && navigationTools.length > 0 && (
              <Separator orientation="vertical" className="h-6" />
            )}
            {renderToolGroup(navigationTools)}
            {navigationTools.length > 0 && collaborationTools.length > 0 && (
              <Separator orientation="vertical" className="h-6" />
            )}
            {renderToolGroup(collaborationTools)}
            {collaborationTools.length > 0 && contentTools.length > 0 && (
              <Separator orientation="vertical" className="h-6" />
            )}
            {renderToolGroup(contentTools)}
          </CardContent>
        </Card>
