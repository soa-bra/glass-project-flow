...
export const TemplatesArchivePanel: React.FC = () => {
  ...
  return <div className="h-full flex flex-col bg-transparent">
      ...
      <div className="px-6 mb-6">
        <div className="p-4 rounded-3xl  bg-transparent">
          <div className="flex items-center gap-4">
            <div className="flex-1 relative">
              <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input type="text" placeholder="البحث في النماذج والقوالب..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pr-12 pl-4 py-3 rounded-full border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/20 font-arabic" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        <div className="space-y-4">
          {mockTemplatesData.map(item => <div key={item.id} className="p-6 rounded-3xl border border-black/10 bg-transparent">
