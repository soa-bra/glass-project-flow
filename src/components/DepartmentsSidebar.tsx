  ...
  return <aside style={{
    ...
    background: '#000000'
      ...
      <nav className="flex flex-col gap-2 h-full py-0 mx-0 px-0">
        ...
        <div className={`text-center mb-2 rounded-full mx-0 px-0 py-[24px] my-[24px] sync-transition ${isCollapsed ? 'flex justify-center' : ''}`}>
          <div className={`flex items-center rounded-lg sync-transition ${isCollapsed ? 'justify-center px-0 mx-0' : 'justify-between px-[3px] mx-[20px]'}`}>
            ...
            transitionDelay: isCollapsed ? '0ms' : 'calc(var(--animation-duration-main) * 0.4)'
              ...
              <h2 className="text-white text-right font-medium text-3xl px-0 mx-[18px] whitespace-nowrap">
                الإدارات
              </h2>
            </div>
            
            <button onClick={toggleSidebar} className="group w-[60px] h-[60px] rounded-full flex items-center justify-center border-2 border-white/30 hover:border-white/60 hover:bg-white/20 hover:shadow-lg flex-shrink-0 sync-transition-fast" style={{ backgroundColor: '#DCE8EE' }}>
              <div className="sync-transition-fast">
                {isCollapsed ? <ChevronRight className="w-6 h-6 text-black sync-transition-fast group-hover:scale-110" /> : <ChevronLeft className="w-6 h-6 text-black sync-transition-fast group-hover:scale-110" />}
              </div>
            </button>
          </div>
        </div>
        ...
        {/* Departments List */}
        <div className="flex flex-col gap-2 flex-1 overflow-y-auto px-[20px]">
          ...
          return <button key={department.key} onClick={() => onDepartmentSelect(department.key)} className={`
                  ...
                  ${isActive ? 'bg-white/25 text-white font-medium rounded-full shadow-lg border border-white/50 scale-[1.02]' : 'text-white/70 hover:bg-white/15 hover:text-white font-light rounded-full hover:shadow-md border border-transparent hover:border-white/30 hover:scale-[1.02] active:scale-95'}
            ...
            transitionDelay: isCollapsed ? '0ms' : `calc(var(--stagger-delay) * ${index})`
                ...
                <div className={`
                    ...
                    ${isActive ? 'bg-white/30 border-white/60 shadow-lg scale-105' : 'border-white/30 group-hover:border-white/60 group-hover:bg-white/20 group-hover:shadow-md group-hover:scale-110 group-active:scale-95'}
                  `}>
