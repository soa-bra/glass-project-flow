        ...
        <div className="flex items-center justify-between px-6 py-[24px] my-[24px]">
          ...
          <div className="w-fit">
            <Tabs defaultValue={content.tabs[0]} dir="rtl" className="w-full">
              <TabsList className="grid w-full bg-white/20 rounded-full p-1" style={{ gridTemplateColumns: `repeat(${content.tabs.length}, 1fr)` }}>
                {content.tabs.map(tab => (
                  <TabsTrigger 
                    key={tab} 
                    value={tab} 
                    className="rounded-full font-arabic text-sm whitespace-nowrap px-4 data-[state=active]:bg-black data-[state=active]:text-white"
                  >
                    {tab}
                  </TabsTrigger>
                ))}
          ...
          <div className="px-0 my-0">
            {content.tabs.map(tab => (
              <TabsContent key={tab} value={tab} className="flex-1 mt-0 overflow-auto px-0 mx-0">
                <div 
                  className="h-full mx-6 mb-6 rounded-2xl overflow-hidden"
                  style={{
