...
const Index = () => {
  ...
  // تحديث متغيرات CSS عند تغيير حالة الشريط الجانبي
  useEffect(() => {
    const root = document.documentElement;
    if (isSidebarCollapsed) {
      root.style.setProperty('--content-area-left', 'calc(var(--sidebar-width-collapsed) + var(--sidebar-margin) + 20px)');
    } else {
      root.style.setProperty('--content-area-left', 'calc(var(--sidebar-width-expanded) + var(--sidebar-margin) + 20px)');
    }
  }, [isSidebarCollapsed]);
