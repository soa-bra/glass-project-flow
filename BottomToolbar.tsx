...
import { useTooling } from "../../state/ToolState";
import type { ToolId } from "../../types/panels";
...
export default function BottomToolbar() {
  ...
  return (
    <div style={{ position:"absolute", bottom:8, left:"50%", transform:"translateX(-50%)", background:"#ffffffd9", backdropFilter:"blur(8px)", border:"1px solid #e5e7eb", borderRadius:12, padding:6, display:"flex", gap:6, zIndex:10 }}>
      {tools.map(t => (
        <button key={t.id} onClick={()=>setActiveTool(t.id)} title={`${t.label} • ${t.shortcut}`} style={{ padding:"6px 10px", fontWeight: activeTool===t.id ? 700 : 400 }}>
